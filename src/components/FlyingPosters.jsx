...
function AutoBind(self, { include, exclude } = {}) {
  ...
  const filter = key => {
    const match = p => (typeof p === 'string' ? key === p : p.test(key));
...
class Media {
  ...
  update(scroll) {
    ...
    if (this.plane.position.y + planeH / 2 < -vpH / 2) this.extra -= this.heightTotal;
    else if (this.plane.position.y - planeH / 2 > vpH / 2) this.extra += this.heightTotal;
  }
}
...
class Canvas {
  ...
  addEventListeners() {
    ...
    /* Scope drag/touch to container so it doesn't swallow page events */
...
export default function FlyingPosters({
  ...
  /* Prevent default scroll on the canvas so the WebGL gets the event */
  useEffect(() => {
